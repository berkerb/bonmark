...
All write commands share the same common flags. The individual subcommands (`regroup`, `rename`, `remove`, `dedup`) are convenience shortcuts — exactly equivalent to `bonmark clean <file> --<operation>`.
...
Triggered by `bonmark clean <file> --interactive` (or `-i`).
...
2. **Niche-clustering pass:** for bookmarks not matched in pass 1, extract signals from the URL and title — TLD patterns, subdomain keywords (e.g. `docs.`, `api.`, `blog.`), path segments, and title word frequency — and cluster them into inferred niche groups. Name each cluster after its most frequent keyword (e.g. "Recipes", "Finance", "Academia").
...
All suffix-stripping patterns must be anchored to end-of-string (`$`) and match against known site names only — never partial matches.
...
Send HTTP HEAD requests to each bookmark URL in parallel (default: 10 workers). Timeout default: 5 s.
...
Some servers reject `HEAD` requests (405 Method Not Allowed) even for live pages. Fall back to a `GET` request with `Range: bytes=0-0` to fetch only the first byte.

### Cross-domain redirect detection

Flag URLs where the final redirect lands on a different domain than the original — commonly dead links pointing to parked domains or ad redirects.

### Resume on interrupted URL check

If the process is interrupted (Ctrl-C, crash), partial results are written to `bonmark_urlcheck_<hash>.tmp` in the system temp directory. On the next run against the same input file:
...
Run before other operations. Normalize URLs before comparison: lowercase scheme and host, strip trailing slash, strip common tracking parameters (`?ref=`, `?utm_*`), normalize `http` → `https`.
...
- Same article syndicated on multiple domains

Present duplicates grouped by URL and ask the user which copy to keep (default: the one with the better title or the most recent `ADD_DATE`).

---

## Netscape Bookmark File format

All major browsers export bookmarks in this format.
...
bonmark looks for a `.bonmarkrc` file in order of precedence:

1. `./.bonmarkrc` — current working directory
2. `~/.bonmarkrc` — user home directory
3. Built-in defaults

CLI flags always take the highest precedence. A specific config file can be pointed to with `--config PATH`.
...
The `[groups]` section is merged with the hardcoded map; conflicts favour the user's config.
...
pip-installable with a single entry point via `pyproject.toml`:

```toml
[project.scripts]
bonmark = "bonmark:main"
```
